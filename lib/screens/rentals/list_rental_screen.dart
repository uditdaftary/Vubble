...
//  LIST RENTAL SCREEN
// ─────────────────────────────────────────────
class ListRentalScreen extends StatefulWidget {
  const ListRentalScreen({super.key});
  @override
  State<ListRentalScreen> createState() => _ListRentalScreenState();
}

class _ListRentalScreenState extends State<ListRentalScreen> {
  ...
  static const _categories = ['Electronics', 'Lab Gear', 'Books', 'Sports', 'Clothing', 'Other'];
      ...
      child: ListView(
        ...
        children: [
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(hintText: 'e.g. Canon EOS 1500D DSLR'),
          ),
          ...
          TextFormField(
            ...
            validator: (v) => (v == null || v.trim().isEmpty) ? 'Describe the item' : null,
            decoration: const InputDecoration(
              hintText: 'Describe condition, included accessories, usage history…',
            ...
            child: Row(children: [
              AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: _noDeposit ? AppColors.cyan : AppColors.border, width: 1.5),
                ),
                child: _noDeposit ? const Icon(Icons.check_rounded, size: 13, color: Colors.black) : null,
              ),
              const SizedBox(width: 8),
              Text('No deposit required', style: AppText.body(size: 13, color: AppColors.textMuted)),
            ]),
          ),
          const SizedBox(height: 20),
          // ── availability window
  ...
  // ── progress ──────────────────────────────────
  Widget _progressBar() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    const SizedBox(height: 8),
    Text('Step $_completedSteps of 4', style: AppText.body(size: 12, color: AppColors.textMuted)),
  ]);
  ...
  Widget _earningsTip() => Container(
    ...
    child: Row(children: [
      ...
      Expanded(child: Text(
        ...
        style: AppText.body(size: 13, color: AppColors.lime.withOpacity(0.85)).copyWith(height: 1.4),
      )),
    ]),
  );
  ...
  Widget _categoryGrid() => GridView.count(
    ...
    children: _categories.map((cat) {
      ...
      final color = RentalCategory.colorOf(cat);
      return GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Text(RentalCategory.emojiOf(cat), style: const TextStyle(fontSize: 14)),
  ...
  // ── rate field ────────────────────────────────
  Widget _rateField() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  // ── deposit field ─────────────────────────────
  Widget _depositField() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    _fieldLabel('DEPOSIT (₹)'),
    const SizedBox(height: 8),
    TextFormField(
      controller: _depositCtrl,
      enabled: !_noDeposit,
      style: GoogleFonts.syne(color: AppColors.amber, fontSize: 20, fontWeight: FontWeight.w800),
      keyboardType: TextInputType.number,
      inputFormatters: [FilteringTextInputFormatter.digitsOnly],
      decoration: InputDecoration(
        ...
        prefixStyle: GoogleFonts.syne(color: AppColors.amber, fontSize: 20, fontWeight: FontWeight.w800),
        fillColor: _noDeposit ? AppColors.border.withOpacity(0.4) : AppColors.surfaceHigh,
      ),
    ),
  ]);
  ...
  // ── live preview ──────────────────────────────
  Widget _livePreview() {
    final cc = _category.isNotEmpty ? RentalCategory.colorOf(_category) : AppColors.cyan;
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: AppColors.cyan.withOpacity(0.06), blurRadius: 16)],
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          if (_category.isNotEmpty)
            CategoryBadge(label: _category, color: cc, emoji: RentalCategory.emojiOf(_category)),
          const Spacer(),
          Container(
        ...
        Row(children: [
          ...
          if (!_noDeposit && _depositCtrl.text.isNotEmpty)
            Text('+ ₹${_depositCtrl.text} dep.', style: AppText.body(size: 12, color: AppColors.textMuted)),
        ]),
      ]),
    );
  }
  ...
  Future<void> _pickDate({required bool isFrom}) async {
    ...
    final init = isFrom ? now : (_availFrom ?? now).add(const Duration(days: 1));
    final first = isFrom ? now : (_availFrom ?? now).add(const Duration(days: 1));
    ...
    if (picked != null) setState(() { if (isFrom) _availFrom = picked; else _availTo = picked; });
  }

  Future<void> _submit() async {
    if (!_formKey.currentState!.validate()) return;
    if (_category.isEmpty) {
      _snack('Pick a category', AppColors.coral); return;
    }
    if (_availFrom == null || _availTo == null) {
      _snack('Set availability window', AppColors.coral); return;
    }
    setState(() => _isListing = true);
    // TODO: FirestoreService.createRentalListing(...)
...
class _DateTile extends StatelessWidget {
  ...
  String get _text => date != null ? '${date!.day}/${date!.month}/${date!.year}' : 'Tap to pick';
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label, style: AppText.label(size: 10, color: date != null ? color : AppColors.textMuted)),
        const SizedBox(height: 6),
        Row(children: [
          Icon(Icons.calendar_today_rounded, size: 14,
            color: date != null ? color : AppColors.textMuted),
          const SizedBox(width: 6),
          Text(_text, style: AppText.body(size: 13, color: date != null ? AppColors.textPrimary : AppColors.textMuted)),
        ]),
      ]),
    ),
  );
